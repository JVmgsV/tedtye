...
const handleTopCenterAction = (event) => {
  if (event) {
    event.preventDefault();
  }
  openPlayExperience(event);
};

if (ui.topCenterAction) {
  ui.topCenterAction.addEventListener("click", handleTopCenterAction);
} else {
  document.addEventListener("click", (event) => {
    if (event.target.closest("#top-center-action")) {
      handleTopCenterAction(event);
    }
  });
}
